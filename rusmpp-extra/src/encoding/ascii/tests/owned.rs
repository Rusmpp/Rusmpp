use crate::{
    ...
    encoding::{
        ascii::{Ascii, AsciiConcatenateError, AsciiEncodeError},
        owned::Encoder,
    },
};
...
mod concatenate {
    ...
    mod error {
        ...
        fn zero_part_size() {
            ...
            assert!(matches!(err, AsciiConcatenateError::PartCapacityExceeded))
        }
        ...
        fn parts_count_exceeded() {
            ...
            assert!(matches!(
                err,
                AsciiConcatenateError::PartsCountExceeded { actual: 256, .. }
            ))
        }
    }
    ...
    fn cases() {
        struct TestCase {
            ...
            expected: Result<&'static [&'static [u8]], AsciiConcatenateError>,
        }
        ...
        let cases: &[TestCase] = &[
            ...
            TestCase {
                ...
                part_header_size: 4,
                expected: Ok(&[&[
                    0x61, 0x67, 0x6A, 0x77, 0x6B, 0x6C, 0x67, 0x6A, 0x6B, 0x77, 0x50, 0x4F,
                ]]),
            },
            TestCase {
                name: "two_parts",
