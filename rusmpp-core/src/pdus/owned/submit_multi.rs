...
impl SubmitMulti {
    ...
    /// Sets the UDH Indicator bit in the GSM Features field of the [`SubmitMulti::esm_class`].
    pub const fn with_udh_indicator(mut self) -> Self {
        self.esm_class = self.esm_class.with_udh_indicator();
        self
    }

    /// Checks wether the UDH Indicator bit is set in the GSM Features field of the [`SubmitMulti::esm_class`].
    ...
    pub fn with_message_payload(mut self, message_payload: MessagePayload) -> Self {
        self.push_tlv(MessageSubmissionRequestTlvValue::MessagePayload(
            message_payload,
        ));
        self
    }
